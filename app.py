...
import pandas as pd
from eplprediction.models.probability_estimator import ProbabilityEstimatorNetwork
from eplprediction.viz.vizualization import Visualizer
...
def league_predictions_figure(league_name, regressor_str, matchdays_to_drop, long_form_vote):
    ...
    database = f'eplprediction/database/{league_name}_database.db'
    database_handler = SQLliteHandler(league=config.league, database=database)
    ...
    with gr.Blocks() as iface:
        ...
        with gr.Row():
            with gr.Column(scale=1, min_width=600):
                slide1 = gr.Slider(minimum=0, maximum=10, step=1, value=3, show_label=True, interactive=True, label="Select number of matchdays to drop")
